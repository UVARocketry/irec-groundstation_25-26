...
<T.PerspectiveCamera makeDefault position={[22, 4.0, 22]} fov={35}>
  <OrbitControls 
    enableZoom={true} 
    target={[0, 3.4, 0]} 
    enableDamping={true} 
...
<T.DirectionalLight position={[10, 20, 10]} intensity={2.5} castShadow />
...
<ContactShadows scale={20} blur={2.5} far={12} opacity={0.15} position.y={-2.98} />